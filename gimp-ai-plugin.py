...
class GimpAIPlugin(Gimp.PlugIn):
    ...
    def _get_api_key(self):
        ...
        if self.config and "openai" in self.config and self.config["openai"].get("api_key"):
    ...
    def _export_image_for_ai(self, image, drawable, max_size=1024):
        ...
        try:
            ...
            if width != max_size or height != max_size:
                print(f"DEBUG: Resizing from {width}x{height} to {max_size}x{max_size} (square)")
            ...
            try:
                ...
                try:
                    ...
                    if not export_success:
                        ...
                        row_data = b'\x00' + b'\x80\x80\x80' * width  # Filter byte + gray RGB pixels
                        ...
                        png_data = (
                            b'\x89PNG\r\n\x1a\n' +                                    # PNG signature
                            len(ihdr_data).to_bytes(4, 'big') + b'IHDR' +            # IHDR chunk length + type
                            ihdr_data + ihdr_crc.to_bytes(4, 'big') +                # IHDR data + CRC
                            len(compressed_data).to_bytes(4, 'big') + b'IDAT' +      # IDAT chunk length + type
                            compressed_data + idat_crc.to_bytes(4, 'big') +          # IDAT data + CRC
                        ...
                        export_success = True
                        print(f"DEBUG: Fallback PNG created: {len(png_data)} bytes for {width}x{height}")
    ...
    def _create_simple_mask(self, width=512, height=512):
        ...
        try:
            ...
            print(f"DEBUG: Created inpainting mask PNG: {len(png_data)} bytes (transparent center circle, white background)")
            return png_data
    ...
    def _create_transparent_mask(self, width=512, height=512):
        ...
        try:
            ...
            row_data = b'\x00' + b'\x00\x00\x00\x00' * width  # Filter byte + transparent RGBA pixels
            image_data = row_data * height
            compressed_data = zlib.compress(image_data)
    ...
    def _call_openai_inpaint(self, image_data, mask_data, prompt, api_key):
        ...
        try:
            ...
            with urllib.request.urlopen(req, context=ctx, timeout=120) as response:
                ...
                print(f"DEBUG: OpenAI API response received: {len(response_data)} bytes")
                ...
                try:
                    ...
                    if temp_img:
                        ...
                        if temp_layers and len(temp_layers) > 0:
                            ...
                            print(f"DEBUG: Source layer dimensions: {source_layer.get_width()}x{source_layer.get_height()}")
                    ...
                    if new_layer is not None:
                        ...
                        layer_x, layer_y = offsets[1], offsets[2]  # Skip the first boolean result
                        
                        print(f"DEBUG: Successfully loaded AI result as layer using PDB procedure")
                        print(f"DEBUG: Layer details - dimensions: {layer_width}x{layer_height}, visible: {layer_visible}, opacity: {layer_opacity}, position: ({layer_x}, {layer_y})")
                        ...
                        for i, layer in enumerate(all_layers):
                            print(f"DEBUG: Layer {i}: '{layer.get_name()}', visible: {layer.get_visible()}")
                    ...
                    return True, f"AI result imported as new layer: '{new_layer.get_name()}' (total layers: {layer_count})"
    ...
    def run_inpaint(self, procedure, run_mode, image, drawables, config, run_data):
        ...
        prompt = self._show_prompt_dialog("AI Inpaint", "fill this area naturally")
        if not prompt:
        ...
        if not api_key:
            Gimp.message("❌ No OpenAI API key found!\n\nPlease set your API key in:\n- config.json file\n- OPENAI_API_KEY environment variable")
            return procedure.new_return_values(Gimp.PDBStatusType.CANCEL, GLib.Error())
        
        # Step 3: Export image and get final dimensions (force to 512x512 for OpenAI)
        success, message, image_data, final_width, final_height = self._export_image_for_ai(image, drawables[0] if drawables else None, max_size=512)
        ...
        if prompt.lower().startswith("test"):
            print("DEBUG: Test mode - using local test image instead of API")
            # Create a mock response that points to a test image
            api_success = True
            api_message = "Test mode - using mock image"
            api_response = {
                "data": [{
            ...
            Gimp.message(f"🔄 Sending request to OpenAI...\n\nPrompt: {prompt}\nThis may take 10-30 seconds...")
            api_success, api_message, api_response = self._call_openai_inpaint(image_data, mask_data, prompt, api_key)
        
        if api_success:
            print(f"DEBUG: AI API succeeded: {api_message}")
            
            # Step 6: Download and import the result
            Gimp.message(f"⬇️ Downloading AI result...\n\nPrompt: {prompt}\nProcessing image...")
            import_success, import_message = self._download_and_import_result(image, api_response)
            
            if import_success:
                Gimp.message(f"✅ AI Inpaint Complete!\n\nPrompt: {prompt}\n{import_message}")
                print(f"DEBUG: Import succeeded: {import_message}")
            else:
                Gimp.message(f"⚠️ AI Generated but Import Failed!\n\nPrompt: {prompt}\nAPI: {api_message}\nImport Error: {import_message}")
